...
import torch
import torchvision
from ..utils.boxes import xywh2xyxy, box_iou

__all__ = ["nms", "non_max_suppression"]
...
def nanodet_nms(
    ...
    prediction: (N, num_obj, xyxy+num_classes), no objectness.
    ...
    assert (
        0 <= min_conf <= 1
    ), f"Invalid Confidence threshold {min_conf}, valid values are between 0.0 and 1.0"
    assert (
        0 <= iou_thres <= 1
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        xc = (conf > min_conf).squeeze()

        box = x[xc][:, :4]  # box
        j = j[xc]  # pred class index
        conf = conf[xc]  # conf
...
def non_max_suppression(
    ...
    """Runs Non-Maximum Suppression (NMS) on inference results
    prediction: (N, num_obj, xywh+objectness+num_classes)
